Neotoma2LiPD = function(site){
  ...
  ## wrap this around the search , such that multiple datasets fo to multiple paleoData and crhonData tables
  allPubs = c()
  ...
  for(n in 1:length(dsIDs)){
    ...
    for(ch in 1:length(neo[[n]]$chronologies)){
      ...
      for(s in 1:length(cNames)){
        ...
        if(!all(is.na(chron[[cNames[s]]]))){
          cmt[[cNames[s]]]$values = chron[cNames[s]]
        ...
        chronData[[ch]]$chronMeasurementTable = cmt
